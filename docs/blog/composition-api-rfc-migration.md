...
이를 약하게 만드는 가장 쉬운 방법은 중간에 무언가(=어댑터)를 껴넣는 것이다.
:::
...
그래서 공식 문서에서는 `ref` 대신에 `reactive`로 상태를 정의하고, `toRefs`를 통해 값을 반환하도록 제안(권고)하고 있다. 
...
setup(props, context) {
  ...
  const onBlur = (): void => {
    state.isFocused = false // state의 property를 통해 value 에 접근한다.
  }
...
앞서 소개한 코드를 통하여 개발을 진행 했을 때 코드베이스가 대폭 증가했다.
