...
또한 이 경험이 레거시 코드 때문에 밤낮고생하는 업계의 소프트웨어 엔지니어에게 조금이나마 도움이 될까해서 공유했다.
...
오픈빌더 코드가 몇년이 지나도 수정이 용이하고 오픈소스의 버전업와 마이그레이션이 용이한 구조를 구성해서 롱런하는 프로젝트를 만들기 위해 시작하였다.
...
**해결방법**은 외부라이브러리의 진입점을 담당하는 서비스를 만들어 해당 서비스를 통해서만 진입하도록 했다.
...
**카카오톡**은 응답하는 방법에 따라 이미지, 텍스트, 리스트, 커머스, 카드, 뮤직이 있다.
이러한 역할 많은 역할을 **응답형식 컴포넌트**에서 많이 담당을 하고 있었다.
...
**첫번째**는 응답형식의 모델을 응답형식 컴포넌트에서 **모두 구체적으로 기술**이 되었다.
구체적으로 기술하기에는 역할이 많아짐으로 **카카오미니와 카카오톡 컴포넌트 두개**로 분리했다.

**두번째**는 응답형식은 최대 3개의 그룹으로 관리되는 데,
그룹 내부의 정보를 모두 기술하는 책임을 가지면
...
**마지막**으로 컴포넌트 네이밍을 수정했다.
컴포넌트 네이밍 컨벤션이 위배되는 부분과 의미를 명확히 알 수 없는 컴포넌트의 네이밍을 변경했다.
