...
- 흥미로운 공개 도메인 패턴에 대한 참조는 TBD(완료)입니다.
...
둘째, 어떻게 시스템을 설계할 수 있는가 하면 사용자가 시스템에 자신의 정신 모델이 반영되어 있는 것을 볼 수 있습니다. 
...
이 계획은 유망하며 사용자 정신 모델과 프로그램을 동시에 발전시키는 데 일반적으로 알려진 언어와 방법의 시작이 될 수 있습니다.
...
P-3: 멘탈 객체 모델(4페이지)과 P-4: 개인 정보 시스템(7페이지)은 사용자의 정신 모델이 사용자 인터페이스를 통해 접근할 수 없는 객체 모델이어야 한다고 제안한다.

##### 문제
도메인 서비스는 효율성과 정보 무결성을 위해 구현되는 반면 최적의 정신 모델은 사용자와 그의 직무에 따라 달라진다. 
도메인 서비스가 다른 사용자의 정신 모델에 해당한다는 보장은 없다. 
...
##### 영향력
- 강력하고 이해할 수 있는 모델링 언어가 사용자의 정신 모델의 생성과 실현을 촉진합니다.
...
1) 목표와 과제를 가진 사용자.
2) 사용자의 정신 모델의 상태, 구조 및 행동을 나타내는 모델.
...
##### 알려진 사용
이 패턴은 OOram 모델링 도구에서 광범위하게 사용되었습니다. 그것은 또한 Exie 제품에도 사용된다[출품하다.
