...
**비추천 방법**은 결국 **일하기 힘들어지는 방법**인데, 일하기 힘든 것이라 하면 시간이 많이 들고 실수할 확률이 높은 방법이라고 할 수 있다.
비추천 방법은 **의미없이 커밋**하는 것이다.

대표적으로 두가지 유형이 있다. 첫번째는 **한 작업의 커밋에 다른 작업을 껴서 커밋**하는 것이다.
코드의 양이 많지 않아 작업이 다른 커밋에 껴서 커밋을 하는 경우이다. 두번째는 사용자 스토리를 무수히 많이 작업한뒤 커밋을 하나만 하는 것이다.
...
기능 개발은 **feature 브랜치**에서 작업한다. **feature 브랜치**는 **develop 브랜치**에서 만들고, 네이밍은 `feature/<기능명 | 이슈 ID>`와 같이한다.
...
D---E---F---G---H---I develop
```
release 브랜치에서 **QA를 진행 완료 후 배포**를 하게 된다. 완료 후에 release 브랜치는 develop과 master 브랜치에 병합되며 배포 태그을 생성하게 된다. 그리고 master 브랜치를 기준으로 서비스 배포를 진행하면 배포가 완료된다.
...
D---E---F---G develop
```
hotfix 브랜치는 master 브랜치에서 만든다. 네이밍은 보편적으로 `hotfix/fix-<기능명>`으로 사용된다. 
...
```
충돌을 수정완료 뒤에는 다시 develop 브랜치에 병합하면 완료 된다.

### 현업에서 발생할만한 상황들
소프트웨어는 이해관계자가 연결되어 항상 나이스한 방향으로만 흐르지 않는다.
...
혼자 작업을 하더라도 feature 브랜치를 나누는 것이 업무의 효율을 높일 수 있다.
소프트웨어는 이해관계자가 많아 요구사항이 변경은 자주 발생된다.

특정 기능이 스펙아웃되거나 다수의 기능중 일부만 배포를 해야 되는 상황이 존재한다.
...
이 상황은 실수로 PR 중인 브랜치로 작업을 하는 경우가 종종 본다.
...
이 상황은 신규 기능 개발은 급하게 필요하지만, develop에는 production에 배포하면 안되는 사항이 있을 때다.
...
```
각 기능의 PR은 `hotfix/YYYYMMDD`에 진행을 한다. 개발 완료 후 내부 테스트가 필요하다면 Sandbox에 배포 시
hotfix/YYYYMMDD로 배포를 한다. 
...
production 배포는 기존 프로세스데로 master로 진행한다.
이렇게 되면 깃플로우 정책을 해치지 않고 작업이 가능하다.

##### 고민이 되었던 부분
이 솔루션을 알기 전까지는 배포를 하는 부분에고 고민을 했다. develop/master를 통해서만 배포를 해야 된다는 생각을 가졌는 데,
