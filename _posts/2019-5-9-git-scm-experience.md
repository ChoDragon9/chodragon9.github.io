...
보편적으로는 **깃플로우** 정책을 많이 따르거나 프로젝트에 맞게 변경해서 사용한다.
...
### 커밋 전략
커밋하는 단위를 쪼개는 작업는 개발자별로 천차만별이다. 

#### 추천하는 방법
**추천하는 방법**은 작은 기능 단위로 커밋을 하는 것이다. 작은 기능 단위는 사람마다 다르게 생각을 할 수 있는 데, 나는 잘개쪼갠 사용자 스토리로 커밋을 한다. 사용자 스토리란 기획서를 기준으로 사용자 입장에서 작성한 기능 단위이다.
...
### 예외상황 대응전략
소프트웨어는 이해관계자가 다수 연결되어 항상 나이스한 방향으로만 흐르지 않는다.
