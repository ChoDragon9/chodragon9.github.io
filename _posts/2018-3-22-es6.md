...
date:   2018-03-22
description: 카카오에서 파트원들에게 발표한 ES6 Overview입니다.
tags: [es6]
---
...
블럭 밖에서 사용을 할 수 없기 때문에 더이상 for loop를 사용하기 전에i와 같은 변수를 정의할 필요없어졌습니다.
```js
for(var i = 0; i < 3; i++) {}
console.log(i); //3
...
### let vs const
블럭 스코트 선언 키워드에서 let 이외에도 const 라는 키워드가 있습니다.
...
만약에 프로퍼티 변경을 막을려면 freeze라는 함수를 사용됩니다.
...
### Arrow function
이번에는 화살표함수입니다. 기존의 함수는 `메소드가 아닌 함수`, `메소드, `생성자` 이 세가지의 기능을했습니다. 이 기능을 위해 `bind(this)`, `self = this`와 같은 코드를 선언해야 했습니다.
하지만 화살표함수는 이 문제를 해결책으로 함수의 기능만 담았습니다.
#### function declaration
기존 함수선언문은 function 키워드를 통해 정의되었습니다.
```javascript
function sum (a, b) {
  return a + b
}
...
화살표함수는 함수 기능만 하기 때문에 생성자로 사용할 수 없습니다.
```javascript
const Person = () => {}
...
#### Object
함수에서 weight, height를 두개의 argument 받는 다고 가정을 했을 때
peter 변수에 있는 값을 사용한다면 weight, height 라는 이름으로 새로 정의하며 가져와서 사용할 수 있습니다.
peter라는 오브텍트에서 weight와 height를 가져와 변수명과 값을 정의하는 의미입니다.

```javascript
// personal.js
const body = () => ({weight: 72, height: 173})
...
```
### Default value
함수와 Destructuring 부분에 기본값을 정의할 수 있습니다.
...
### ...
점점점이라는 신텍스가 추가 됬는 데, 이 신텍스는 두가지 기능이 있습니다.

#### Rest Parameter
Rest Operator 기능은 함수로 전달되는 argument들중
변수로 정의되지 않는 것들을 모두 가져옵니다.
...
Spread 기능은 반복가능한 데이터 앞에 ...을 사용하면
아이템들을 순서대로 꺼내줍니다.
